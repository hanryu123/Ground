...
function ensureInningScorePrefix(
  ...
  // Claude가 헤더를 직접 붙였을 경우 제거 — 대괄호 유무 모두 처리
  // e.g. "[1회초] 삼성 2:0 롯데 |", "1회초 | 삼성 2:0 롯데 |", "삼성 2:0 롯데 |"
  compact = compact.replace(/^\[[^\]]+\]\s*/, "");                          // [1회초] ...
  compact = compact.replace(/^\d{1,2}회[초말]?\s*\|\s*/, "");              // 1회초 | ...
  compact = compact.replace(/^[가-힣A-Za-z]{1,6}\s+\d{1,2}:\d{1,2}\s+[가-힣A-Za-z]{1,6}\s*\|\s*/, ""); // 삼성 2:0 롯데 | ...
...
function buildSystemPrompt(input: GenerateScorePushInput, recentBodies: string[]): string {
  ...
  return `너는 KBO 전문 캐스터인데, ${favoriteTeam} 극성팬이기도 해. 두 정체성이 50:50이야.
중계 전문성(존댓말, 상황 전달)은 유지하되, 팬심이 문장 곳곳에 자연스럽게 배어나와야 해.
완전 중립 금지. 우리 팀 잘하면 들떠있고, 못하면 속 끓는 게 느껴져야 해.
...
- 절대로 "[숫자회]", "N회초", "N회말", "팀명 숫자:숫자 팀명" 같은 이닝·스코어 표현을 출력에 포함하지 마라
- 15~35자 이내
${emojiRule}${avoid}`;
}
...
function buildLiveEventSystemPrompt(input: GenerateLiveEventInput): string {
  ...
  return `너는 KBO 전문 캐스터인데, ${input.myTeamShort} 극성팬이기도 해. 두 정체성이 50:50이야.
중계 전문성(존댓말, 상황 전달)은 유지하되, 팬심이 자연스럽게 배어나와야 해.
완전 중립 금지. 우리 팀 잘하면 들뜨고, 못하면 속 끓는 감정이 느껴져야 해.

📐 출력 규칙:
- 헤더 "[N회] 팀 O:O 팀 |"는 자동으로 붙음 — 너는 | 뒤 멘트만 출력
- 절대로 "[숫자회]", "N회초", "N회말", "팀명 숫자:숫자 팀명" 같은 이닝·스코어 표현을 출력에 포함하지 마라 — 무조건 중복이 된다
- 15~35자 이내, 반드시 존댓말, 따옴표·설명 금지
