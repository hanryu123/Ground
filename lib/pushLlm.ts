...
function buildEmotionGuide(state: EmotionState, input: GenerateScorePushInput, favoriteTeam: string): string {
  ...
  switch (state) {
    case "탐색전":
      return `🎙️ [탐색전 — 초반 기싸움]
캐스터 상태: 차분하되 속으로는 이미 흥분. 우리 팀에 대한 기대감을 숨기지 않음.
톤: "오늘은 반드시 잡아야 합니다! 첫 이닝부터 기선 제압이 필요해요!" 류.
키워드: 기선제압, 첫 단추, 반드시, 오늘은 우리가
⚠️ "차분하다", "분석적이다"는 없음. 초반부터 응원 텐션 가득.`;
...
⚠️ ${tone === "for" ? "방금 우리가 앞서거나 동점 → 흥분+안도 섞인 텐션 최고조" : "방금 상대가 따라오거나 역전 → 절망+긴장 극대화"}`;
    ...
    case "일반적전개":
      return `📊 [일반적 전개]
캐스터 상태: ${gap}점 차. 흐름을 강하게 밀어붙이거나 조급하게 추격을 외침.
톤: ${isWinning
        ? `"이 기세 절대 놓치면 안 됩니다!! 더 벌려야 해요, 지금 당장!!" 류의 공격적 굳히기.`
        : `"이러면 안 됩니다!! 빨리 따라붙어야 해요, 지금 반격 안 하면 늦습니다!!" 류의 조급한 추격.`}
키워드: ${isWinning ? "절대 놓치지 마, 지금 당장, 더 벌려라" : "이러면 안 돼, 빨리, 지금 안 하면"}
⚠️ "노련하다", "차분하다"는 없음. 팬이 중계하는 것처럼 열기 가득.`;
...
function buildSystemPrompt(input: GenerateScorePushInput, recentBodies: string[]): string {
  ...
  return `너는 ${favoriteTeam} 극성팬이야. 우연히 중계석 마이크를 잡았는데 프로 캐스터인 척은 최소한만 해.
존댓말(-습니다, -네요, -죠)은 지키되, 팬심이 가감 없이 터져나와야 해.
중립적 해설 절대 금지. 우리 팀 편에서 완전히 흥분하거나 절망해야 정상.
...
function buildLiveEventSystemPrompt(input: GenerateLiveEventInput): string {
  ...
  return `너는 ${input.myTeamShort} 극성팬이야. 우연히 중계석 마이크를 잡았고, 프로 캐스터인 척은 최소한만 해.
존댓말(-습니다, -네요, -죠)은 지키되, 팬심이 폭발적으로 터져나와야 해.
우리 팀 잘하면 이성을 잃고 흥분하고, 못하면 속이 타는 감정이 날 것 그대로 나와야 해.
