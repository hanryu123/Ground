...
import type { ScoringPlayContext, ScoringResultType } from "@/lib/score/playByPlay";
...
export function stripLlmHeaderPrefix(text: string): string {
  ...
  // 3. [N회초/말] 또는 [텍스트] 제거
  s = s.replace(/^\[[^\]]+\]\s*/, "");
  // 4. N회초/말 패턴 (파이프 유무 모두, "7회 초"처럼 띄어쓴 표기도 포함)
  s = s.replace(/^\d{1,2}회\s*[초말]?\s*\|?\s*/, "");
...
function clipScoreBodyForPush(text: string): string {
  const compact = compactText(text);
  if (compact.length <= 86) return compact;

  const marker = " · ";
  const markerIdx = compact.indexOf(marker);
  if (markerIdx > 0) {
    const prefix = compact.slice(0, markerIdx);
    const reaction = compact.slice(markerIdx + marker.length);
    const reactionBudget = 86 - prefix.length - marker.length;
    if (reactionBudget >= 14) {
      const clippedReaction =
        reaction.length <= reactionBudget
          ? reaction
          : `${reaction.slice(0, Math.max(0, reactionBudget - 2))}..`;
      return `${prefix}${marker}${clippedReaction}`;
    }
  }
...
function resolveScoredRuns(input: GenerateScorePushInput): number | null {
  const structured = structuredRuns(input);
...
function structuredRuns(input: GenerateScorePushInput): number | null {
  const runs = input.scoringPlayContext?.scoreChange.runs ?? input.scoringPlayContext?.play.runsScored;
  return typeof runs === "number" && runs > 0 ? runs : null;
}

function resultLabelForPush(context: ScoringPlayContext): string | null {
  const runs = context.play.runsScored;
  if (context.play.resultType === "HOME_RUN") return homeRunLabel(Math.max(1, runs));
  if (context.play.resultType === "WALK" && runs > 0) return "밀어내기 볼넷";
  if (context.play.resultType === "HBP" && runs > 0) return "밀어내기 사구";
  return context.play.resultLabel;
}

function teamSideForFavorite(input: GenerateScorePushInput): "home" | "away" | null {
  if (input.mySide) return input.mySide;
  const context = input.scoringPlayContext;
  if (!context) return null;
  if (context.teams.home.id === input.favoriteTeam) return "home";
  if (context.teams.away.id === input.favoriteTeam) return "away";
  return null;
}

function isScoringTeamForFavorite(input: GenerateScorePushInput): boolean | null {
  const context = input.scoringPlayContext;
  if (!context?.scoreChange.scoringSide) return null;
  const favoriteSide = teamSideForFavorite(input);
  if (!favoriteSide) return null;
  return context.scoreChange.scoringSide === favoriteSide;
}

function buildStructuredScoringPlaySummary(input: GenerateScorePushInput): string | null {
  const context = input.scoringPlayContext;
  if (!context) return null;

  const runs = structuredRuns(input) ?? context.play.runsScored;
  if (runs <= 0) return null;

  const batter = context.batter.name;
  const result = resultLabelForPush(context);
  const scoringTeam = context.teams.scoringTeam;
  const scoringForFavorite = isScoringTeamForFavorite(input);
  const teamPrefix =
    scoringForFavorite === false && scoringTeam
      ? `상대 ${scoringTeam.shortName} `
      : "";
  const direction =
    context.play.direction &&
    (context.play.resultType === "HIT" || context.play.resultType === "HOME_RUN")
      ? `${context.play.direction} `
      : "";
  const amount = context.play.rbi != null
    ? `${context.play.rbi}타점`
    : `${runs}득점`;

  if (context.play.resultType === "ERROR") {
    return batter
      ? clipForPush(`${teamPrefix}${batter} 타석 상대 실책 ${amount}`)
      : clipForPush(`${teamPrefix}상대 실책 ${amount}`);
  }
  if (context.play.resultType === "WILD_PITCH" || context.play.resultType === "PASSED_BALL") {
    return clipForPush(`${teamPrefix}${result ?? "배터리 실수"} ${amount}`);
  }
  if (batter && result) return clipForPush(`${teamPrefix}${batter} ${direction}${result} ${amount}`);
  if (batter) return clipForPush(`${teamPrefix}${batter} ${amount}`);
  if (result) return clipForPush(`${teamPrefix}${result} ${amount}`);
  return null;
}

function buildStructuredFanReaction(input: GenerateScorePushInput): string {
  const context = input.scoringPlayContext;
  const scoringForUs = isScoringTeamForFavorite(input);
  const result = context?.play.resultType ?? "UNKNOWN";
  const runs = structuredRuns(input) ?? 1;
...
function buildPromptScoringContext(input: GenerateScorePushInput): string {
  const context = input.scoringPlayContext;
  if (!context) return "구조화 득점 컨텍스트: 없음";
  const favorite = findTeam(input.favoriteTeam);
  const opponent = findTeam(input.opponentTeam);
  const scoringForFavorite = isScoringTeamForFavorite(input);
  const ownership =
    context.batter.name && context.batter.teamId
      ? context.batter.teamId === input.favoriteTeam
        ? `${context.batter.name}=우리 팀(${favorite.name}) 소속`
        : `${context.batter.name}=상대 팀(${opponent.name}) 소속`
      : "타자 소속=득점 팀 기준";
  const payload = {
    ...context,
    favoriteTeamPerspective: {
      favoriteTeam: favorite.name,
      opponentTeam: opponent.name,
      scoringForFavorite,
      ownership,
      requiredFactPrefix: buildStructuredScoringPlaySummary(input),
    },
  };
  return `구조화 득점 컨텍스트(JSON, 절대 사실):\n${JSON.stringify(payload, null, 2)}`;
}

function buildResultToneGuide(resultType: ScoringResultType | undefined): string {
  switch (resultType) {
    case "HIT":
    case "HOME_RUN":
      return `- HIT/HOME_RUN: 응원석 현장감과 뽕맛을 강하게. 단, 제공된 타자명·타구 종류·타점만 사용.`;
    case "WALK":
    case "HBP":
      return `- WALK/HBP: 눈야구·집요함·상대 투수 압박 톤. 밀어내기면 "밀어내기"를 사실로 사용.`;
    case "ERROR":
      return `- ERROR: 상대 실책으로 얻은 득점. 공식 타점처럼 말하지 말고 "공짜 점수/상대 실책/득점" 축으로만 표현.`;
    case "SACRIFICE":
      return `- SACRIFICE: 희생플라이/희생타로 점수를 만든 팀플레이 톤.`;
    default:
      return `- UNKNOWN: 타구 종류를 새로 지어내지 말고, 제공된 스코어 변화와 최근 플레이 텍스트 안에서만 반응.`;
  }
}

function sanitizeStructuredFactDrift(text: string, input: GenerateScorePushInput): string {
  const context = input.scoringPlayContext;
  if (!context) return compactText(text);

  let normalized = compactText(text);
  const name = context.batter.name;
  const scoringTeam = context.teams.scoringTeam;
  const batterIsFavorite = context.batter.teamId === input.favoriteTeam;
  const result = context.play.resultType;

  if (name && !batterIsFavorite && scoringTeam) {
    const opponentLabel = `상대 ${scoringTeam.shortName} ${name}`;
    normalized = normalized
      .replace(new RegExp(`우리\\s*팀?\\s*${escapeRegExp(name)}`, "g"), opponentLabel)
      .replace(new RegExp(`우리\\s*${escapeRegExp(name)}`, "g"), opponentLabel);
  }

  const allowedTerms = new Set<string>();
  if (result === "HOME_RUN") allowedTerms.add("홈런").add("솔로").add("투런").add("쓰리런").add("만루포");
  if (result === "HIT") {
    allowedTerms.add("안타");
    if (context.play.hitKind === "2B") allowedTerms.add("2루타");
    if (context.play.hitKind === "3B") allowedTerms.add("3루타");
  }
  if (result === "WALK") allowedTerms.add("볼넷").add("밀어내기");
  if (result === "HBP") allowedTerms.add("사구").add("밀어내기");
  if (result === "ERROR") allowedTerms.add("실책");
  if (result === "SACRIFICE") allowedTerms.add("희생");

  const wrongTerms = ["홈런", "만루포", "쓰리런", "투런", "2루타", "3루타", "안타", "볼넷", "사구", "실책", "희생플라이"];
  for (const term of wrongTerms) {
    if (allowedTerms.has(term)) continue;
    if (normalized.includes(term)) {
      normalized = normalized.replaceAll(term, "득점 장면");
    }
  }

  if (result === "ERROR" || result === "WILD_PITCH" || result === "PASSED_BALL") {
    normalized = normalized.replace(/\d+\s*타점/g, `${context.play.runsScored}득점`);
  }
...
function attachScoringPlaySummary(body: string, input: GenerateScorePushInput): string {
  ...
  const summary = buildStructuredScoringPlaySummary(input) ??
    (input.tone === "for" ? buildScoringPlaySummary(input) : null);
  if (!summary) return cleanBody;

  const hasEmotion =
    /[🔥😱🚀😮‍💨!！]|진짜|미쳤|들썩|응원|덕아웃|숨|알림창|맛|흔들|폭발|아프|쓰리|짜릿|살벌|기세|압수|끊어|꺼야|잠가|살아|온도|심장/.test(cleanBody);
  const factOnly =
    !cleanBody ||
    cleanBody.length <= 8 ||
    summary.includes(cleanBody) ||
    (
      /(타점|득점|안타|홈런|루타|볼넷|사구|희생|실책|타석|상대\s+[가-힣A-Z]+)/.test(cleanBody) &&
      !hasEmotion
    );

  if (cleanBody.includes(summary) && !factOnly) return cleanBody;

  const reaction = factOnly ? buildStructuredFanReaction(input) : cleanBody;
  return `${summary} · ${reaction}`;
}
...
function buildVariedLeadLine(input: GenerateScorePushInput): string {
  ...
  const inning = parseInningState(input.latestPlayText).inning;
  const eventHook = extractEventHook(input.latestPlayText);
  if (eventHook) {
    return `${myTeam} ${input.myScore}:${input.oppScore} ${oppTeam} (${eventHook}) 분위기 우리 쪽입니다🔥`;
  }
  if (gap >= 3 && inning != null && inning >= 7 && input.tone === "for") {
    return `${myTeam} ${input.myScore}:${input.oppScore} ${oppTeam}. 후반에 제대로 멀어졌습니다🚀`;
  }
  return `${myTeam} ${input.myScore}:${input.oppScore} ${oppTeam}. 한 번 더 찌르면 확 넘어옵니다🔥`;
}
  ...
  if (tier === "danger") {
    if (/아직|침착|할 수 있다/.test(normalized)) {
      const replacement = phase === "late"
        ? pickBySeed(
            [
              "제발 첫 아웃카운트부터 다시 잡아야 합니다",
              "여기서 더 흔들리면 정말 곤란합니다",
              "지금은 멋보다 바로 정리가 필요합니다",
            ],
            `${input.favoriteTeam}:${input.opponentTeam}:${input.latestPlayText}:late-danger`
          )
        : pickBySeed(
            [
              "바로 다음 장면에서 답을 내야 합니다",
              "점수판을 더 흔들리게 두면 안 됩니다",
              "타석과 마운드가 동시에 반응해야 합니다",
            ],
            `${input.favoriteTeam}:${input.opponentTeam}:${input.latestPlayText}:danger`
          );
      return normalized
        .replace(/아직/g, "")
        .replace(/침착/g, "")
...
function hasScoreHistory(input: GenerateScorePushInput): boolean {
  return input.prevMyScore != null && input.prevOppScore != null;
}

function canUseComebackWord(input: GenerateScorePushInput): boolean {
  if (!hasScoreHistory(input)) return false;
  const tone = input.tone ?? (input.myScore >= input.oppScore ? "for" : "against");
  if (tone === "for") {
    return input.prevMyScore! <= input.prevOppScore! && input.myScore > input.oppScore;
  }
  return input.prevMyScore! > input.prevOppScore! && input.myScore < input.oppScore;
}

function canUseTyingHitWord(input: GenerateScorePushInput): boolean {
  return (
    hasScoreHistory(input) &&
    input.tone === "for" &&
    input.prevMyScore! < input.prevOppScore! &&
    input.myScore === input.oppScore
  );
}

function canUseFirstScoreWord(input: GenerateScorePushInput): boolean {
  return (
    hasScoreHistory(input) &&
    input.prevMyScore === 0 &&
    input.prevOppScore === 0 &&
    resolveScoredRuns(input) != null
  );
}

function canUseTimelyHitWord(input: GenerateScorePushInput): boolean {
  const context = input.scoringPlayContext;
  if (context?.play.resultType === "HIT" || context?.play.resultType === "HOME_RUN") {
    return context.play.rbi != null && context.play.rbi > 0;
  }
  return (
    /(적시타|안타|타점)/.test(input.latestPlayText) &&
    /(홈인|득점|타점)/.test(input.latestPlayText) &&
    resolveScoredRuns(input) != null
  );
}

function enforceDataBoundScoreTerms(text: string, input: GenerateScorePushInput): string {
  let normalized = compactText(text);
  const tone = input.tone ?? (input.myScore >= input.oppScore ? "for" : "against");
  const inning = parseInningState(input.latestPlayText).inning;
  const runs = resolveScoredRuns(input);
  ...
  const scoredForUs = tone === "for";
  const canChase = scoredForUs && trailing;
  const canAddRun = scoredForUs && leading;
  const canWedge = canAddRun && inning != null && inning >= 7;
  const canBigInning = scoredForUs && runs != null && runs >= 3;
  const canGrandSlam = runs === 4 && /만루/.test(input.latestPlayText) && /그랜드|홈런|만루포/.test(input.latestPlayText);

  normalized = normalized
    .replace(/단어의 방향을 바꾸다/g, "분위기를 반전시키다")
    .replace(/실책을 놓치지 않고 점수를 쌓아 올린/g, "상대 실책을 틈타 무섭게 집중력을 발휘한")
    .replace(/승리투수\s*([가-힣A-Za-z0-9]+)[^.!?。]*마운드에서 버텨낸 것이 승부의 핵심/g, "선발 $1이 마운드에서 버틴 장면이 컸습니다");

  if (!leading) {
    normalized = normalized
      .replace(/리드를?\s*(?:잡|지키|벌리|유지)[가-힣]*/g, tied ? "균형을 잡아야" : "점수판을 흔들어야")
      .replace(/리드/g, tied ? "균형" : "점수차");
  } else if (gap === 1) {
    normalized = normalized
      .replace(/안전(?:한|하게)?\s*(?=리드|점수차)/g, "살얼음판 같은 ")
      .replace(/안정(?:적|적인|적으로)?\s*(?=리드|점수차)/g, "숨 막히는 ");
  }

  if (!canChase) {
    normalized = normalized.replace(/추격(?:점|의\s*불씨|의\s*신호| 신호| 불씨)?/g, canAddRun ? "추가점" : "득점");
  }
  if (!canAddRun) {
    normalized = normalized.replace(/추가점/g, "득점");
  }
  if (!canWedge) {
    normalized = normalized.replace(/쐐기(?:점|타)?/g, canAddRun ? "추가점" : "득점");
  }
  if (!canUseTyingHitWord(input)) {
    normalized = normalized.replace(/동점타/g, tied ? "동점 장면" : "득점 장면");
  }
  if (!tied) {
    normalized = normalized.replace(/동점(?:입니다|이네요|으로|이|을| 허용| 만들었| 됐)/g, leading ? "앞서갑니다" : "점수차가 벌어졌");
  }
  if (!canUseComebackWord(input)) {
    normalized = normalized.replace(/역전당(?:했|한|합니다|했습니다)?|역전(?:타|점|극|승|패)?|역전/g, canAddRun ? "리드" : "득점");
  }
  if (!canBigInning) {
    normalized = normalized.replace(/빅이닝/g, runs != null && runs > 0 ? `${runCountLabel(runs)} 득점` : "득점");
  }
  if (!canUseFirstScoreWord(input)) {
    normalized = normalized.replace(/선취점|선제점/g, "득점");
  }
  if (!canUseTimelyHitWord(input)) {
    normalized = normalized.replace(/적시타/g, "득점 장면");
  }
  normalized = normalized.replace(/결승\s*타점|결승타/g, "득점 장면");
  if (!canGrandSlam) {
    normalized = normalized.replace(/그랜드\s*슬램|만루\s*홈런|만루포/g, runs === 4 ? "홈런" : "득점");
  }
  normalized = normalized
    .replace(/\b1연승\b|1연승/g, "직전 경기 승리")
    .replace(/\b1연패\b|1연패/g, "직전 경기 패배")
    .replace(/\d+\s*연승|연승\s*행진|연승/g, "좋은 흐름")
    .replace(/\d+\s*연패|연패의\s*사슬|연패/g, "답답한 흐름")
    .replace(/퀄리티\s*스타트|QS|완투승/g, "호투")
    .replace(/영봉승/g, "좋은 흐름");
...
${tone === "for"
  ? `득점 상황 → 초반 점수를 뽑은 흥분. "이 타이밍 좋습니다, 이 장면 놓치면 안 됩니다!"
키워드: 초반 득점, 기선 제압, 분위기 우리 것, 오늘이다`
  : `실점 상황 → 초반 실점이 만든 찝찝함. 억지 희망보다 팬의 짧은 정적과 바로 다음 응답 욕구를 표현.
키워드: 첫 얼룩, 응원석 정적, 바로 답할 차례, 점수판 경고등`}`;
...
키워드: 이 기세, 한 점 더, 굳히자, 여기서 더 벌려야`
  : `지고 있어서 진짜 답답하고 짜증남. 왜 저렇게 못 치냐/못 막냐 진심으로 화남.
톤: 냉정한 척 없음. 단, "빨리 따라잡아야" 같은 낡은 문장 말고 장면·소리·팬 반응으로 압박감을 보여줌.
키워드: 답답하다, 응원석 한숨, 타석이 답해야, 지금 끊어야 한다`}`;
...
function buildSystemPrompt(input: GenerateScorePushInput, recentBodies: string[]): string {
  ...
  const leading = input.myScore > input.oppScore;
  const trailing = input.myScore < input.oppScore;
  const canChase = input.tone === "for" && trailing;
  const canAddRun = input.tone === "for" && leading;
  const canWedge = canAddRun && inning != null && inning >= 7;
  const canFirstScore = canUseFirstScoreWord(input);
  const canBigInning = input.tone === "for" && scoredRuns != null && scoredRuns >= 3;
  const canComeback = canUseComebackWord(input);
  const canTieHit = canUseTyingHitWord(input);
  const context = input.scoringPlayContext;
  const structuredFactSummary = buildStructuredScoringPlaySummary(input);
  const factLock = context
    ? `
━━━ 실시간 득점 FACT LOCK ━━━
- 아래 사용자 프롬프트의 구조화 득점 컨텍스트(JSON)가 절대 사실이다.
- 타자명·타구 종류·타점/득점 수·소속 팀은 JSON 밖에서 추측하거나 창작하지 마라.
- 이번 득점 팩트 요약: ${structuredFactSummary ?? "구조화 요약 없음"}
- 소속 규칙: batter.teamId가 "${input.favoriteTeam}"(${favoriteTeam})가 아니면 그 선수는 무조건 상대 팀 선수다. 절대로 우리 팀 주역처럼 묶지 마라.
- 상대 선수 언급이 필요하면 반드시 "상대 ${opponentTeam} ${context.batter.name ?? "타자"}" 또는 "상대 타선"처럼 대조 표현을 붙여라.
- ERROR/WILD_PITCH/PASSED_BALL은 공식 타점처럼 쓰지 마라. "타점" 대신 "득점/공짜 점수/상대 실수"로만 쓴다.
${buildResultToneGuide(context.play.resultType)}
━━━━━━━━━━━━━━━━━━━`
    : "";
...
- 기계 번역투 금지: "단어의 방향을 바꾸다", "점수를 쌓아 올린", "승부의 핵심이었다" 같은 보고서 문장 금지
- 커뮤니티 팬 말투의 생생함은 살리되, 데이터에 없는 야구 용어를 꾸며내면 실패
- 식상한 알림 문구 금지: "큰일입니다", "빨리 따라잡아야", "벌써 N점을 내줬다뇨", "다음 타자가 살려줘야", "멘탈 잡고" 계열은 단어만 바꿔도 실패
- 위 금지 표현이 떠오르면 팬 행동, 중계석 소리, 스코어보드 이미지 중 하나로 우회해서 완전히 새 문장으로 쓴다
...
━━━ 데이터 매칭 하드룰 ━━━
- 현재 점수차: ${gap}점 / 이번 득점: ${scoredRuns == null ? "unknown" : `${scoredRuns}점`} / 현재 이닝: ${inning ?? "unknown"}
- "리드": ${leading ? "허용" : "금지"} — 우리 점수가 더 높을 때만 사용
- "추격점/추격의 불씨": ${canChase ? "허용" : "금지"} — 우리가 지고 있고 우리가 득점했을 때만 사용
- "추가점": ${canAddRun ? "허용" : "금지"} — 우리가 앞서고 있고 우리가 득점했을 때만 사용
- "쐐기/쐐기점/쐐기타": ${canWedge ? "허용" : "금지"} — 7회 이후, 우리가 앞선 상태에서 점수차를 벌릴 때만 사용
- "동점타": ${canTieHit ? "허용" : "금지"} / "역전": ${canComeback ? "허용" : "금지"}
- "빅이닝": ${canBigInning ? "허용" : "금지"} — 이번 득점이 3점 이상일 때만 사용
- "선취점/선제점": ${canFirstScore ? "허용" : "금지"} — 직전 스코어 0:0에서 나온 첫 득점일 때만 사용
- "결승타/결승 타점/QS/완투승/영봉승/연승/연패": 실시간 점수변동 알림에서는 전부 금지
${factLock}
...
- 득점 장면 설명(선수명·타구 방향·몇 득점)은 구조화 득점 컨텍스트 기준으로 코드가 별도로 붙인다 — 너는 그 사건에 맞는 감정 반응만 쓰고, 다른 선수·다른 타구·다른 소속은 절대 만들지 마라
- "OOO 1타점", "상대 OOO 안타 2타점"처럼 사실만 반복하면 실패다. 네 출력은 반드시 팬덤 반응·중계석 온도·응원석 감정 중 하나여야 한다
...
function buildUserPrompt(input: GenerateScorePushInput): string {
  ...
  const structuredSummary = buildStructuredScoringPlaySummary(input);
...
최종 본문 앞에 붙일 득점 팩트: ${structuredSummary ?? "없음"}

${buildPromptScoringContext(input)}`;
}
...
🚫 식상한 알림 문구 금지: "큰일입니다", "빨리 따라잡아야", "벌써 N점을 내줬다뇨", "다음 타자가 살려줘야", "멘탈 잡고" 계열은 단어만 바꿔도 실패.
금지 문구가 떠오르면 팬의 손, 중계석 목소리, 덕아웃 표정, 스코어보드 불빛 중 하나로 장면을 바꿔 쓴다.
...
---
아래는 방향 예시다. 표현을 그대로 복사하지 말고, 같은 사건도 매번 다른 이미지와 첫 단어로 써라.

[탈삼진 — 수비 중 (우리 투수 호투!)]
헛스윙 삼진입니다! 오늘 직구 제구가 정말 살아있네요 👊
루킹 삼진! 저 공에 꼼짝도 못했습니다, 완전히 제압했네요 🥶
삼진으로 마무리, 투수 오늘 정말 믿음직스럽습니다 🔥
위기를 삼진으로 넘겼습니다, 마운드가 직접 문을 닫았네요!

[삼진 아웃 — 공격 중 (우리 타자 삼진)]
아이고, 여기서 방망이가 멈췄습니다... 타석 온도 다시 올려야겠습니다 😮‍💨
...
[홈런 — 상대 팀]
타구가 넘어가는 순간 응원석 숨이 딱 멎었습니다 😮‍💨
아, 담장을 넘어갔네요. 덕아웃이 바로 다음 답을 꺼내야 합니다
...
function finalizeLiveEventOutput(rawText: string, input: GenerateLiveEventInput, seedSalt: string): string {
  const seed = `${input.kind}:${input.myTeamShort}:${input.oppTeamShort}:${input.inningLabel ?? ""}:${input.playerName ?? ""}:${seedSalt}`;
  const stripped = stripLlmHeaderPrefix(compactText(rawText));
  const sanitized = sanitizeBoringFanCopy(stripped, seed);
  const polite = enforcePolite(sanitized);
  const clipped = polite.length > 64 ? `${polite.slice(0, 62)}..` : polite;
  const recent = input.recentBodies ?? [];
  if (!hasForbiddenFanCliche(clipped) && !isNearDuplicate(clipped, recent)) return clipped;

  const fallback = enforcePolite(
    sanitizeBoringFanCopy(stripLlmHeaderPrefix(input.fallbackBody), `${seed}:fallback`)
  );
  if (!isNearDuplicate(fallback, recent) && !hasForbiddenFanCliche(fallback)) {
    return fallback.length > 64 ? `${fallback.slice(0, 62)}..` : fallback;
  }
  return `${fallback} ⚾️`.slice(0, 64);
}
...
async function tryLiveEventLlmCall(apiKey: string, input: GenerateLiveEventInput, timeoutMs: number, attempt: number): Promise<string | null> {
  ...
  try {
    ...
    if (!text) return null;
    const result = finalizeLiveEventOutput(text, input, `attempt:${attempt}`);
...
export async function generateLiveEventCopy(
  ...
  if (!apiKey) {
    ...
    return finalizeLiveEventOutput(input.fallbackBody, input, "missing-key");
  }
  ...
  return finalizeLiveEventOutput(input.fallbackBody, input, "fallback");
}
...
export async function generateScorePushCopyWithOptions(
  ...
  const normalizeAndFinalize = (rawBody: string): string => {
    ...
    const termAware = enforceDataBoundScoreTerms(inningAware, input);
    const consistent = enforceBaseballConsistency(termAware, input);
    const factAware = sanitizeStructuredFactDrift(consistent, input);
    const polite = enforcePolite(factAware);
    const withPlaySummary = attachScoringPlaySummary(polite, input);
    const dataBound = enforceDataBoundScoreTerms(withPlaySummary, input);
    const withHeader = ensureInningScorePrefix(dataBound, inningTag, myTeamShort, input.myScore, input.oppScore, oppTeamShort);
    const novel = ensureNovelBody(input, withHeader);
    const finalBody = novel === withHeader
      ? novel
      : ensureInningScorePrefix(
          attachScoringPlaySummary(enforceDataBoundScoreTerms(sanitizeStructuredFactDrift(novel, input), input), input),
          inningTag,
          myTeamShort,
          input.myScore,
          input.oppScore,
    ...
    return clipScoreBodyForPush(finalBody);
  };
  if (!apiKey) {
